...
layui.use(['laydate', 'laypage', 'layer', 'table', 'element', 'slider'], function(){
  ...
  table.render({
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'id', title: 'ID', width:180, sort: true, fixed: 'left', totalRowText: '合计：'}
    	 ...
    	 //脚本编辑弹出层
	      ...
	      // console.log(data.toolName);
	        layer.open({
	           ...
	           maxmin: true,
	           area: ['70%', '70%'],
	           content: '${pageContext.request.contextPath}/jsp/studentInfo.jsp?toolSceneId='+data.toolSceneId+'&'+'id='+data.id,
    		 ...
    		 //脚本编辑弹出层
    	      ...
    	      // console.log(data.toolName);
    	        layer.open({
    	           ...
    	           maxmin: true,
    	           area: ['70%', '70%'],
    	           content: '${pageContext.request.contextPath}/jsp/studentEdit.jsp?toolSceneId='+data.toolSceneId+'&'+'id='+data.id,
    	           success: function(layero, index){  
    	              var body = layer.getChildFrame('body', index);  
    	               var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();  
    	               var inputList = body.find("input");
